...
这些就是 Gtk 组件，无需显式导入即可使用。例如 `<box />` 和 `<Gtk.Box />` 是完全相同的。
...
```tsx
<label
  label="<span foreground='blue'>文本</span>"
  useMarkup
  wrap
...
<revealer
  ...
  revealChild={true}
  onNotifyChildRevealed={() => print("动画完成")}
>
  <Child />
</revealer>
```
...
> [!WARNING]
>
> 在 Gtk4 中，窗口默认不可见，所以不要忘记传递 `visible`。
...
```tsx
<entry
  onChanged={({ text }) => print("文本已更改", text)}
  onActivate={({ text }) => print("回车", text)}
/>
```
...
```tsx
<label label="你好" maxWidthChars={16} wrap />
```
